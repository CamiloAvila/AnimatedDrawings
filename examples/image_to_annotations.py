...
def image_to_annotations(img_fn: str, out_dir: str) -> None:
    ...
    resp = requests.post(f"http://{torchserve_host}:8080/predictions/drawn_humanoid_detector", files=request_data, verify=False)
    ...
    torchserve_host = os.getenv("TORCHSERVE_HOST", "localhost")
    resp = requests.post(f"http://{torchserve_host}:8080/predictions/drawn_humanoid_pose_estimator", files=data_file, verify=False)
    if resp is None or resp.status_code >= 300:
